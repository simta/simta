...
#
#   The option --with-db=DIR can be used to specify a specific Berkeley
...
#
#   Changes from upstream: sets rpath, --with-db instead of --with-bdb-dir
#
# LICENSE
#
#   Copyright (c) 2008 Tim Toolan <toolan@ele.uri.edu>
#   Copyright (c) 2013 Paul Arthur MacIain <paul.maciain@flowerysong.com>
#
...
AC_DEFUN([AX_PATH_BDB], [
  ...
  if test "x$with_db" = "x" ; then
    ...
    AX_PATH_BDB_NO_OPTIONS([$1], [HIGHEST], [
     ...
     # Set --with-db option.
     ax_path_bdb_INC="$with_db/include"
     ax_path_bdb_LIB="$with_db/lib"
     ...
     LDFLAGS="-L$ax_path_bdb_LIB -Wl,-rpath -Wl,$ax_path_bdb_LIB $LDFLAGS"
     ...
     if test -f "$ax_path_bdb_INC/db.h" ; then
       ...
       AX_PATH_BDB_NO_OPTIONS([$1], [ENVONLY], [
         ...
         BDB_LDFLAGS="-L$ax_path_bdb_LIB -Wl,-rpath -Wl,$ax_path_bdb_LIB"
       ])
     else
       AC_MSG_RESULT([no])
...
AX_PATH_BDB_STUFF DB_VERSION_MAJOR,DB_VERSION_MINOR,DB_VERSION_PATCH
	...
	if test "x$ax_path_bdb_path_find_highest_DIR" != x ; then
	  ...
	  BDB_LDFLAGS="-L$ax_path_bdb_path_find_highest_DIR/lib -Wl,-rpath -Wl,$ax_path_bdb_path_find_highest_DIR/lib"
	fi
...
AC_DEFUN([AX_PATH_BDB_PATH_GET_VERSION], [
  ...
  if test -d "$1/include" && test -d "$1/lib" &&
    ...
    LDFLAGS="-L$1/lib -Wl,-rpath -Wl,$1/lib $LDFLAGS"
