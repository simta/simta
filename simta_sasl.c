...
int
simta_sasl_server_auth(
        struct simta_sasl *s, const char *mech, const char *resp) {
    int          rc;
    int          proprc;
    yastr        buf = NULL;
    ...
    if (resp) {
        ...
        if (mech && (strcmp(resp, "=") == 0)) {
            buf[ 0 ] = '\0';
        } else if (sasl_decode64(resp, strlen(resp), buf, yaslAllocSize(buf),
                           &buflen) != SASL_OK) {
    ...
    if (mech) {
        rc = sasl_server_start(
                s->s_conn, mech, resp ? buf : NULL, buflen, &out, &outlen);
    } else {
    ...
    proprc = sasl_getprop(
            s->s_conn, SASL_USERNAME, (const void **)&s->s_auth_id);
...
simta_sasl_reset(struct simta_sasl *s, int tls) {
    ...
    if ((rc = sasl_setprop(s->s_conn, SASL_SSF_EXTERNAL, &ssf)) != SASL_OK) {
        syslog(LOG_ERR,
    ...
    if ((rc = sasl_setprop(s->s_conn, SASL_SEC_PROPS, &secprops)) != SASL_OK) {
        syslog(LOG_ERR, "Liberror: simta_sasl_reset sasl_setprop SEC_PROPS: %s",
                sasl_errdetail(s->s_conn));
        return (1);
    }
...
simta_sasl_mechlist(struct simta_sasl *s, const char **result) {
    if (sasl_listmech(s->s_conn, NULL, "", " ", "", result, NULL, NULL) !=
            SASL_OK) {
