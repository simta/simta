...
spf_check_host(struct spf *s, const yastr domain) {
    ...
    for (i = 1; i < tok_count; i++) {
        ...
        if (strncasecmp(split[ i ], "redirect=", 9) == 0) {
            ...
            simta_debuglog(2, "SPF %s [%s]: redirect to %s", s->spf_domain,
                    domain, redirect);
         ...
         */
            ...
            simta_debuglog(2, "SPF %s [%s]: matched all: %s", s->spf_domain,
                    domain, spf_result_str(qualifier));
            ret = qualifier;
            goto cleanup;
         ...
         */
            ...
            if ((domain_spec = spf_macro_expand(s, domain, split[ i ])) ==
                    NULL) {
            ...
            if ((domain_spec = spf_parse_domainspec_cidr(
                         s, domain, split[ i ], &cidr, &cidr6)) == NULL) {
                /* Macro expansion failed, probably a syntax problem. */
                ret = SPF_RESULT_PERMERROR;
                goto cleanup;
            }
            ...
            if ((domain_spec = spf_parse_domainspec(s, domain, split[ i ])) ==
                    NULL) {
                /* Macro expansion failed, probably a syntax problem. */
                ret = SPF_RESULT_PERMERROR;
                goto cleanup;
         ...
         */
            ...
            if ((rc = simta_cidr_compare(
                         cidr, s->spf_sockaddr, NULL, split[ i ])) < 0) {
                syslog(LOG_WARNING,
...
spf_macro_expand(struct spf *s, const yastr domain, const yastr macro) {
    ...
    for (p = macro; *p != '\0'; p++) {
        ...
        case '{':
            ...
            if ((rtransform == 1) || (dtransform > 0) || (delim != '\0')) {
                if (delim == '\0') {
                    delim = '.';
                }
